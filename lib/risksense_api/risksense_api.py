...
class RiskSenseApi:
    ...
    def set_default_client_id(self, client_id:int):

        """Sets the default client id .
        
        Examples:   
            Set a ``client_id`` using :obj:`self.{risksenseobject}.set_default_client_id` function

            >>> apiobj = self.{risksenseobject}.set_default_client_id({client_id})`

        Args:
            client_id: if not supplied, takes default client id
        """
    ...
    def refresh_my_clients(self):
        ...
        :raises RequestFailed,StatusCodeError,MaxRetryError:
        :raises StatusCodeError:
        :raises MaxRetryError:
        """
